...
一个本地/内网运行的全球经济指标实时监控看板，整合展示美联储利率、美国通胀、加密货币行情、A股/美股指数、中国央行利率、中国房价等 32 类核心经济指标。采用 **Dell 1996 目录式设计语言**——纯黑页面边框、扁平色块丝带卡片、Arial Black 粗体标题、Times Roman 衬线正文。
...
| A股自选股雷达（估值/财务/风险事件） | Tushare | 30 分钟缓存 |
| 中国利率与流动性（SHIBOR/LPR） | Tushare + AkShare | 6 小时缓存 |
| 两融余额（沪深合计） | Tushare | 6 小时缓存 |
| 军工行业指数（中证军工 × 上证） | Tushare | 1 小时缓存 |
...
- 默认军工核心 6 支（中兵红箭/电科芯片/北方国际/北化股份/内蒙一机/湖南天雁），支持搜索沪深 A 股自定义添加
- 通过后端通达信（mootdx）接口获取分钟级行情，点击查看 K 线走势
- 自选股列表本地持久化（localStorage）
- **交易时段感知**：开市（北京时间 9:15–11:30 / 12:55–15:05）自选股 **5 秒近实时**刷新（后端一次批量 TDX 请求），休市自动降频（行情 60s→10min、指数 1h→6h、美股 5min→1h），后端休市时段对行情做 5 分钟 TTL 缓存，避免无效轮询；不含法定节假日判断
- **原地刷新不闪现**：轮询期间保留上次数据就地更新数字（价格变动闪红/绿提示），错误时显示上次成功数据而非清空面板

### Tushare 扩展模块

配置 `TUSHARE_TOKEN` 后启用（`backend/.env`），各模块按 Tushare 积分权限逐项降级，无权限的分区显示提示不影响整体：

- **A股自选股雷达** — 自选股的估值（PE/PB/换手/市值，daily_basic）、最新财务指标（EPS/ROE/毛利率/净利同比，fina_indicator）与风险事件（股权质押、限售解禁预告、大宗交易、龙虎榜命中）
- **中国利率与流动性** — SHIBOR 隔夜/3月/1年（Tushare）+ LPR 1年/5年历史（AkShare）及近期调整表
- **两融余额** — 沪深两市融资融券余额按日加总趋势（SSE+SZSE）
- **军工行业指数** — 中证军工（399967）× 上证指数近 1 年净值对比（期初=100）

### 界面功能

- **Dell 1996 目录风格** — 黑色页面框、8 色色块丝带卡片、Arial Black 标题、Times 正文、直角、Dell 红仅用于 CTA/实时角标、黄色 NEW! 贴纸
- **刷新秒出** — query 缓存持久化到 localStorage（Date 安全序列化，maxAge 24h），刷新先水合上次数据立即渲染，再后台静默刷新（stale-while-revalidate）
- **瀑布流自适应布局** — 模块按内容高度自动紧凑排布，无固定行概念，最大化利用空间，支持自由拖拽重排
- **涨红跌绿** — 遵循中国股市配色习惯（涨=Dell 红，跌=绿）
- **历史走势图表** — 顶部时间范围选择器（1月/3月/6月/1年/全部）联动所有趋势图的时间窗口，dataZoom 拖拽缩放；月度/季度宏观数据在短窗口下自动回退全量避免空图
